...
const
    ...
    x_API = new class {
        constructor() {
            ...
            Object.assign( self, {
                ...
                // ※ これを使用しても、一定時間内のリクエスト回数に制限有り→参考: [xのAPI制限 [2019/11/17現在] - Qiita](https://qiita.com/mpyw/items/32d44a063389236c0a65)
                
                // x API には Rate Limit があるため、続けてコールする際に待ち時間を挟む必要あり（15分毎にリセットされる）
                ...
                x_API_DELAY_SHORT : ( ( 15 * 60 ) / 900 + 1 ) * 1000,
                x_API_DELAY_LONG : ( ( 15 * 60 ) / 180 + 1 ) * 1000,
                x_API_DELAY_VERY_LONG : ( ( 15 * 60 ) / 75 + 1 ) * 1000,
            ...
            Object.assign( self, {
                API_DEFINITIONS : {
                    [ TIMELINE_TYPE.user ] : {
                        ...
                        min_delay_ms : self.x_API_DELAY_SHORT,
                        max_retry : 3,
                    },
                    ...
                    [ TIMELINE_TYPE.media ] : {
                        ...
                        min_delay_ms : self.x_API_DELAY_LONG,
                        max_retry : 3,
                    },
                    ...
                    [ TIMELINE_TYPE.likes_legacy ] : {
                        ...
                        tweet_number : { default : 20, limit : 200 },
                        min_delay_ms : self.x_API_DELAY_VERY_LONG,
                        max_retry : 3,
                    },
        ...
        async fetch_timeline_common( timeline_type, url, options ) {
            ...
            do {
                await wait( ( retry_number <= 0 ) ? wait_msec : ( self.x_API_DELAY_VERY_LONG * retry_number ) );
        ...
        create_api_header( api_url ) {
            ...
            return {
                ...
                'x-x-client-language' : self.client_language,
            };
        ...
        get_remain_time_msec_until_next_call( timeline_type ) {
            let remain_time_msec = this.api_called_infos[ timeline_type ].last_time_msec + ( this.API_DEFINITIONS[ timeline_type ].min_delay_ms || this.x_API_DELAY_LONG ) - Date.now();
    ...
    TIMELINE_TOOLBOX = new class {
        ...
        async get_user_timeline_info( options ) {
            ...
            let user_id = options.user_id,
                ...
                count = options.count,
                json = await x_API.fetch_user_timeline( user_id, screen_name, max_id, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_user_timeline() error:', error );
                    return null;
                } );
        ...
        async get_search_timeline_info( query, options ) {
            ...
            let count = options.count,
                max_retry_if_empty_result = options.max_retry_if_empty_result || x_API.API_DEFINITIONS[ TIMELINE_TYPE.search ].max_retry_if_empty_result, // TODO: x側の問題で検索条件に合致するツイートが存在するにも関わらず結果が0で返ることがある→暫定的にリトライすることで対処
                original_delay_min_delay_ms = x_API.API_DEFINITIONS[ TIMELINE_TYPE.search ].min_delay_ms,
                json,
                tweet_info_list = [];
            
            for (let retry_count=0; retry_count <= max_retry_if_empty_result; retry_count++) {
                if (0 < retry_count) {
                    x_API.API_DEFINITIONS[ TIMELINE_TYPE.search ].min_delay_ms = x_API.x_API_DELAY_SHORT; // 外部から間隔が調整されている場合でも、リトライ時には本来の間隔に戻す
                }
                json  = await x_API.fetch_search_timeline( query, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_user_timeline() error:', error );
                    return null;
                } );
                x_API.API_DEFINITIONS[ TIMELINE_TYPE.search ].min_delay_ms = original_delay_min_delay_ms;
        ...
        async get_notifications_timeline_info( options ) {
            ...
            let max_id = options.max_id, // 注：この max_id はツイートIDではなくUNIX時間（ミリ秒）であることに注意
                count = options.count,
                json = await x_API.fetch_notifications_timeline( max_id, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_notifications_timeline() error:', error );
                    return null;
                } );
        ...
        async get_likes_legacy_timeline_info( options ) {
            ...
            let user_id = options.user_id,
                ...
                max_id = options.max_id,
                count = options.count,
                json = await x_API.fetch_likes_legacy_timeline( user_id, screen_name, max_id, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_likes_legacy_timeline() error:', error );
                    return null;
                } );
        ...
        async get_likes_timeline_info( options ) {
            ...
            let user_id = options.user_id,
                ...
                count = options.count,
                json = await x_API.fetch_likes_timeline( user_id, cursor, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_likes_timeline() error:', error );
                    return null;
                } );
        ...
        async get_bookmarks_timeline_info( options ) {
            ...
            let user_id = options.user_id,
                ...
                cursor = options.cursor,
                count = options.count,
                json = await x_API.fetch_bookmarks_timeline( cursor, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_bookmarks_timeline() error:', error );
                    return null;
                } );
        ...
        async get_media_timeline_info( options ) {
            ...
            let user_id = options.user_id,
                ...
                cursor = options.cursor,
                count = options.count,
                json = await x_API.fetch_media_timeline( user_id, cursor, count ).catch( ( error ) => {
                    log_error( 'x_API.fetch_media_timeline() error:', error );
                    return null;
                } );
    ...
    ClassTimelineTemplate = class {
        ...
        get delay_time_msec_until_next_call() {
            return ( 0 < this.tweet_info_list.length ) ? x_API.get_remain_time_msec_until_next_call( this.current_api_type ) / this.tweet_info_list.length : 0;
        } // end of get delay_time_msec_until_next_call()
        
        get api_called_info() {
            return x_API.called_info( this.current_api_type );
    ...
    ClassUserTimeline = class extends ClassTimelineTemplate {
        ...
        async fetch_tweets_from_user_timeline() {
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_user_timeline_info( {
                    ...
                    count : x_API.definition( TIMELINE_TYPE.user ).tweet_number.limit,
                } ).catch( ( error ) => {
        ...
        async fetch_tweets_from_media_timeline() {
            ...
            if ( ! user_info ) {
                user_info = self.user_info = await x_API.get_user_info( { user_id : self.user_id, screen_name : self.screen_name } );
            }
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_media_timeline_info( {
                    ...
                    cursor : self.cursor,
                    count : x_API.definition( TIMELINE_TYPE.media ).tweet_number.limit,
                } ).catch( ( error ) => {
        ...
        async fetch_tweets_from_search_timeline() {
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_search_timeline_info( self.search_query, {
                    count : x_API.definition( TIMELINE_TYPE.search ).tweet_number.limit,
                } ).catch( ( error ) => {
    ...
    ClassNotificationsTimeline = class extends ClassTimelineTemplate {
        ...
        async fetch_tweets_from_notifications_timeline() {
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_notifications_timeline_info( {
                    ...
                    count : x_API.definition( TIMELINE_TYPE.notifications ).tweet_number.limit,
                } ).catch( ( error ) => {
    ...
    ClassLikesLegacyTimeline = class extends ClassTimelineTemplate {
        ...
        async fetch_tweets_from_likes_legacy_timeline() {
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_likes_legacy_timeline_info( {
                    screen_name : self.screen_name,
                    max_id : self.max_tweet_id,
                    count : x_API.definition( TIMELINE_TYPE.likes_legacy ).tweet_number.limit,
                } ).catch( ( error ) => {
    ...
    ClassLikesTimeline = class extends ClassTimelineTemplate {
        ...
        async fetch_tweets_from_likes_timeline() {
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_likes_timeline_info( {
                    ...
                    user_icon : user_info.profile_image_url_https,
                    cursor : self.cursor,
                    count : x_API.definition( TIMELINE_TYPE.likes ).tweet_number.limit,
                } ).catch( ( error ) => {
    ...
    ClassBookmarksTimeline = class extends ClassTimelineTemplate {
        ...
        async fetch_tweets_from_bookmarks_timeline() {
            ...
            let result = self.last_result = await TIMELINE_TOOLBOX.get_bookmarks_timeline_info( {
                    ...
                    user_icon : user_info.profile_image_url_https,
                    cursor : self.cursor,
                    count : x_API.definition( TIMELINE_TYPE.bookmarks ).tweet_number.limit,
                } ).catch( ( error ) => {
